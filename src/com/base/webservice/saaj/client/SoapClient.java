package com.base.webservice.saaj.client;
...
//		MessageFactory mFactory = MessageFactory.newInstance();
		// 也可用如下方式获取消息工厂
		QName serviceName = new QName("http://service.base.com/", "HelloService");
		QName portName = new QName("http://service.base.com/", "HelloServicePort");
		...
		// 创建XML的根元素
		SOAPBodyElement bodyElementRoot = body.addBodyElement(new QName("http://server.cfx.webservice.base.com/", "printContext", "ns1"));
	...
	public static void selectCustomerByName() throws Exception{
		...
		// 创建XML的根元素
		SOAPHeaderElement headerElementRoot = header.addHeaderElement(new QName("http://server.cfx.webservice.base.com/", "c", "ns1"));
		SOAPBodyElement bodyElementRoot = body.addBodyElement(new QName("http://server.cfx.webservice.base.com/", "selectCustomerByName", "ns1"));
		...
		while (it.hasNext()) {
			...
			OutputStream ous = new FileOutputStream("d:\\aaa.jpg");
			while (ins.read(b) != -1) {
				ous.write(b);
			}
			ous.close();
	...
	public static void selectMaxAgeCustomer() throws Exception{
		...
		// 创建XML的根元素
		SOAPBodyElement bodyElementRoot = body.addBodyElement(new QName("http://server.cfx.webservice.base.com/", "selectMaxAgeCustomer", "ns1"));
		...
		// 创建附件对象
		AttachmentPart attachment = message.createAttachmentPart(new DataHandler(new FileDataSource("d:\\c1.jpg")));
		// 设置Content-ID
...
//                "cid:" + attachment.getContentId());
		...
		AttachmentPart attachment2 = message.createAttachmentPart(new DataHandler(new FileDataSource("d:\\c2.jpg")));
...
//		// 输出SOAP消息中的第一个子元素的元素名称
		...
		while (it.hasNext()) {
			...
			OutputStream ous = new FileOutputStream("d:\\bbb.jpg");
			while (ins.read(b) != -1) {
				ous.write(b);
			}
			ous.close();
