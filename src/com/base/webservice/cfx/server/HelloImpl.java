 ...
 *
 */
@WebService(serviceName="HelloService",portName="HelloServicePort",targetNamespace="http://service.base.com/",endpointInterface="com.base.webservice.cfx.server.Hello")
public class HelloImpl implements Hello {
	...
	public Customer selectCustomerByName(Customer customer) {
		if("why".equals(customer.getName())){
			...
			customer.setImageData(new DataHandler(new FileDataSource(new File("d:"+ File.separator + "why.jpg"))));
		}else{
			customer.setId(2);
			customer.setBirthday(new Date());
			customer.setImageData(new DataHandler(new FileDataSource(new File("d:"+ File.separator + "origin.jpg"))));
		}
		return customer;
	}
	...
	public Customer selectMaxAgeCustomer(Customer c1, Customer c2) {
		try {
			...
			OutputStream os = new FileOutputStream("d:\\temp1.jpg");
			...
			os = new FileOutputStream("d:\\temp2.jpg");
			bytes = new byte[1024];
			while ((c = is.read(bytes)) != -1) {
				os.write(bytes, 0, c);
			}
