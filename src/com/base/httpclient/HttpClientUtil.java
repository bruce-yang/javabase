...
public class HttpClientUtil {
	 ...
	 * 发送HTTPS_POST请求
	 * @see 该方法会自动关闭连接,释放资源
	 * @see 该方法默认设置的连接超时时间和读取超时时间,均配置在了config.properties配置文件中,单位为毫秒,超时后方法会自动返回空字符串
	 * @see 该方法会自动对<code>params</code>中的[中文][|][ ]等特殊字符进行<code>URLEncoder.encode(string,encodeCharset)</code>
	 * @param reqURL        请求地址
	 * @param params        请求参数
	 * @param encodeCharset 编码字符集,编码请求数据时用之,其为null时默认采用UTF-8解码
	...
	public static String sendPostSSLRequest(String reqURL, Map<String, String> params, String encodeCharset, String decodeCharset){
		...
		HttpClient httpClient = new DefaultHttpClient();
		httpClient.getParams().setParameter(CoreConnectionPNames.CONNECTION_TIMEOUT, 3000); //连接超时,即URL连接的等待时间
		httpClient.getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT, 3000);              //读取超时,即URL连接成功后,获取Response的等待时间
